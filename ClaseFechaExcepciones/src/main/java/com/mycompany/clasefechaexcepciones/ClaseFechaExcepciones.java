...
public class ClaseFechaExcepciones {
    ...
    public ClaseFechaExcepciones(ClaseFechaExcepciones otraFecha){
        this.anio = anio;
        this.dia = dia;
        this.mes = mes; 
    }
    ...
    public void setAnio(int anio) throws IllegalArgumentException {
        if (anio >= 1900) {
            if (esFechaValida(dia,mes,this.anio))
            this.anio = anio;
        } else {
    ...
    // Método main para probar la clase
      public static void main(String[] args) {
        ...
        try {
            ...
            ClaseFechaExcepciones fecha = new ClaseFechaExcepciones(scanner.nextInt(), scanner.nextInt(), scanner.nextInt());
            fecha.mostrarFecha();
        } catch (IllegalArgumentException e) {
            System.out.println("Error, dato no válido: " + e.getMessage());
        } catch (Exception e) {
